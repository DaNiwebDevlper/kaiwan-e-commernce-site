...
const ProductCard = ({ id, title, imgSrc, price, quantity, width = 'w-full' }) => {
    const dispatch = useAppDispatch()
    return (
        <div className={`sm:w-[250px] ${width} shrink-0 flex flex-col gap-3 transition-all duration-300 transform border rounded-xl p-3`}>
            ...
            <div className="flex justify-between px-1 items-center">
                ...
                <p className='flex items-center'> <span>{quantity == 0 ? '' : <p className='text-emerald-500'>Rs: </p>}</span> {quantity == 0 ? <p className='text-rose-500'>Sold out</p> : price}</p>
            </div>
